# Week 1: Generative AI Use Cases, Project Lifecycle & Pre-training

> Understanding transformers, prompting, and how LLMs are pre-trained at scale.

---

## Lessons

| # | Lesson | Status | SVGs |
|---|--------|--------|------|
| 00 | [Course Introduction](00-course-introduction.md) | 🟢 | 2 |
| 01 | [Introduction - Week 1](01-introduction.md) | 🟢 | 1 |
| 02 | [Generative AI & LLMs](02-generative-ai-llms.md) | 🟢 | 1 |
| 03 | [LLM Use Cases & Tasks](03-llm-use-cases.md) | 🟢 | 1 |
| 04 | [Text Generation Before Transformers](04-before-transformers.md) | 🟢 | 1 |
| 05 | [Transformers Architecture](05-transformers-architecture.md) | 🟢 | 1 |
| 06 | [Generating Text with Transformers](06-generating-text.md) | 🟢 | 1 |
| 07 | [Prompting & Prompt Engineering](07-prompting.md) | 🟢 | 1 |
| 08 | [Generative Configuration](08-generative-config.md) | 🟢 | 1 |
| 09 | [GenAI Project Lifecycle](09-project-lifecycle.md) | 🟢 | 1 |
| 10 | [Pre-training LLMs](10-pretraining-llms.md) | 🔴 | — |
| 11 | [Computational Challenges](11-computational-challenges.md) | 🔴 | — |
| 12 | [Multi-GPU Strategies](12-multi-gpu.md) | 🔴 | — |
| 13 | [Scaling Laws](13-scaling-laws.md) | 🔴 | — |
| 14 | [Domain Adaptation](14-domain-adaptation.md) | 🔴 | — |

---

## Key Topics

- **Transformer Architecture:** Self-attention, multi-headed attention
- **Prompt Engineering:** Context window, completion, inference
- **Generative Configuration:** Temperature, top-k, top-p
- **Pre-training:** Training objectives, compute requirements
- **Scaling Laws:** Chinchilla, compute-optimal models

---

## Lab 1: Summarize Dialogue

Using FLAN-T5 to summarize conversations with different prompting strategies.

---

## 🔗 Navigation

← [Course Overview](../README.md) · → [Week 2: Fine-tuning](../week-2-finetuning/README.md)
